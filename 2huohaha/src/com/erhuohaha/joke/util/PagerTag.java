...
public class PagerTag extends TagSupport {
	...
	private String getNavigatorBar()
	{
		...
		//如果是第一页
		if(curpage == 1){
			bar.append("<span class='current prev'>上一页</span>")
		...
		if(curpage == pages){
			bar.append("<span class='current'>"+pages+"</span>")
			   .append("<span class='current next'>下一页</span>");
		}else{
			bar.append("<a href='"+urlstart+pages+urlend+"'>"+pages+"</a>")
			   .append("<a href='"+urlstart+(curpage+1)+urlend+"'>下一页</a>");
		}
		bar.append("<select style='height:26px;width:100px;text-align: center;padding: 0.3em 0.5em;' onchange=\"location.href='"+urlstart+"'+this.value+'"+urlend+"'\">");
